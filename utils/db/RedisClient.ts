...
class RedisClient {
    ...
    public async AppendToList(
        ...
        try {
            await this._client.append(key, value);
        } catch (ex) {
            this._logger.Error("Unable to write to redis", ex);
            throw ex;
        }
    ...
    public async RemoveFromSet(
        ...
        try {
            ...
            this._logger.Error(
                "Unable to delete value {0} from key {1} from redis",
                value,
                key,
                ex
            );
    ...
    public async SetKeyToHashSet(
        key: RedisModule.KeyType,
        field: string,
        value: RedisModule.ValueType
    ) {
        this._logger.Info(
            "Adding value {0} to hashset {1} with hashkey {2}",
            value,
            key,
            field
        );
    ...
    public async GetKeyFromHashSet(
        key: RedisModule.KeyType,
        field: string
    ): Promise<string | null> {
    ...
    public async RemoveKeyFromHashSet(key: RedisModule.KeyType, field: string) {
        ...
        try {
            ...
            this._logger.Error(
                "Unable to delete field {0} from key {1} from redis",
                field,
                key,
                ex
            );
    ...
    public async AddToSortedSet(
        ...
        try {
            ...
            this._logger.Error(
                "Failed to add {0} to sorted set {1}",
                value,
                key,
                ex
            );
    ...
    public async GetItemsFromSortedSet(
        key: RedisModule.KeyType,
        numItems: number = 1
    ): Promise<Array<string>> {
    ...
    public async RemoveItemFromSortedSet(
        key: RedisModule.KeyType,
        items: RedisModule.ValueType
    ) {
        this._logger.Info(
            "Removing item {0} from sorted set with {1}",
            items,
            key
        );
        try {
            ...
            this._logger.Error(
                "Failed to remove item {0} from set {1}",
                items,
                key
            );
            throw ex;
    ...
    private Disconnect() {
        ...
        if (
            this._subscriberClient != undefined &&
